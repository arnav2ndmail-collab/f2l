...
async def _filelist(update, ctx, uid, page):
    ...
    if not files:
        ...
        for f in files:
            link = (
                ...
                f"?start=file_{f['short_id']}"
            )

            name = escape_md(f['file_name'])
    ...
    if page > 0:
        nav.append(
            InlineKeyboardButton(
                "⬅️",
                callback_data=f"myfiles:{page-1}"
            )
        )
...
def main():
    ...
    application.add_handler(
        MessageHandler(ANY_FILE, handle_file)
    )
